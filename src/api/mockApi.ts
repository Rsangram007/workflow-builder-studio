...
export const api = {
  ...
  simulate: async (workflow: Workflow): Promise<SimulationResult> => {
    ...
    if (errors.length > 0) {
      ...
      console.log("POST /simulate - Response:", result);
      return result;
    }
    ...
    for (const node of executionOrder) {
      ...
      const step: SimulationStep = {
        ...
        title:
          "title" in nodeData ? (nodeData.title as string) : nodeData.label,
        status: "completed",
        message: "",
        timestamp: timestamp,
      };
      ...
      switch (nodeData.type) {
        case "start":
          ...
          break;
        case "task":
          step.message = `Task "${nodeData.title}" assigned to ${
    ...
    const result = { success: true, steps, errors: [] };
    console.log("POST /simulate - Response:", result);
    return result;
  },
};
